...
class HuggingFaceChecker(BaseGrammarChecker):
    ...
    def correct(self, text: str) -> dict:
        ...
        with torch.no_grad():
            outputs = HuggingFaceChecker._model.generate(
                inputs.input_ids,
                max_length=512,
                num_beams=1,  # Giảm beam xuống 1 (Greedy Search) để nhanh nhất có thể
                early_stopping=True
            )
    ...
    #         suggestion_list = corr_words[j1:j2]
    #
    #         # Logic gộp từ ngắn (như 'a ice' -> 'an ice')
    #         is_common_word_error = (i2 - i1) <= 2 and any(len(w) < 4 for w in bad_segment_list)
    #         if is_common_word_error:
    #             if i1 > 0 and j1 > 0: i1 -= 1; j1 -= 1
    #             if i2 < len(orig_words) and j2 < len(corr_words): i2 += 1; j2 += 1
    #             bad_segment_list = orig_words[i1:i2]
    #             suggestion_list = corr_words[j1:j2]
    #
    #         bad_segment = " ".join(bad_segment_list)
    #         suggestion = " ".join(suggestion_list)
    #
    #         if self.is_clean_match(bad_segment, suggestion): continue
    #
    #         # Tạo message lỗi
    #         msg = "Grammar issue"
    #         type_err = "grammar"
    #
    #         if tag == 'delete':
    #             msg = "Từ thừa (Redundant)"; type_err = "deletion"
    #         elif tag == 'insert':
    #             msg = "Thiếu từ (Missing word)"; type_err = "insertion"
    #         elif tag == 'replace':
    #             msg = "Lỗi ngữ pháp/Từ vựng"; type_err = "correction"
    #
    #         errors.append({
    #             "type": type_err,
    #             "original": bad_segment,
    #             "suggestions": [suggestion],  # List suggestions chuẩn format
    #             "message": msg
    #         })
    #
    #     return errors
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            ...
            if tag == 'insert':
                ...
                if i1 > 0:
                    ...
                    # Nếu lỗi ở ngay đầu câu (hiếm), lấy từ phía sau làm mốc
                    pass

                    # 2. Xử lý trường hợp DELETE (Thừa từ) -> Suggestion sẽ là chuỗi rỗng
            elif tag == 'delete':
